...
| 브랜치명   | 설명 |
|------------|------|
| `main`     | 안정적인 최종 배포 버전만 존재하는 브랜치입니다. |
| `dev`      | 기능 개발을 위한 통합 브랜치입니다. 모든 `feature/*` 브랜치들은 이곳에서 파생되고 병합됩니다. |
| `feature/*`| 개별 기능 개발 브랜치입니다. 기능 단위로 생성되며, 작업 완료 후 `dev`로 병합됩니다. |
| `release`  | 배포 전 최종 테스트 및 QA를 위한 브랜치입니다. 테스트가 완료되면 `main`과 `prod`에 병합됩니다. |
| `prod`     | 실제 운영 배포에 사용되는 브랜치입니다. `main`에서 병합된 내용을 기반으로 배포합니다. |
---
